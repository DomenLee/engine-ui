...
test('show area component', async () => {
  ...
  const field = wrapper.findAll('.ghb-field').at(2);
  field.trigger('click');
...
test('set-default', () => {
  const { wrapper } = createComponent();
  wrapper.find('.ghb-switch').trigger('click');
...
test('on change detail', () => {
  const wrapper = mount(AddressEdit);
  const field = wrapper.findAll('.ghb-field__control').at(3);
...
test('show search result', async () => {
  ...
  const items = wrapper.findAll('.ghb-icon-location-o');
